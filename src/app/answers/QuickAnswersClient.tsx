    ...
    return (
        <section className="mt-16">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <h2 className="text-2xl font-bold text-neutral-900">Quick Answers Library</h2>
                <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                    ...
                    <select
                        ...
                        className="rounded-lg border border-neutral-300 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                    >
                ...
                <div className="space-y-4">
                    {filtered.map((item) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className="bg-white rounded-xl border border-neutral-100 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4 shadow-sm"
                            >
                                <div className="flex-1">
                                    ...
                                    <Link href={pageHref} className="block">
                                        <p className="font-semibold text-neutral-900 hover:text-primary-600">
                                            {item.question}
                                        </p>
                                    </Link>
                                ...
                                <div className="flex flex-col items-stretch gap-2 w-full md:w-auto md:min-w-[160px]">
                                    <Link
                                        href={pageHref}
                                        className="inline-flex items-center justify-center rounded-full border border-neutral-200 px-4 py-2 text-xs font-semibold text-neutral-700 hover:border-primary-500 hover:text-primary-600 transition-colors"
                                    >
                                        View full guide
                                    </Link>
                                    {quizHref && (
                                        <Link
                                            href={quizHref}
                                            className="inline-flex items-center justify-center rounded-full bg-primary-600 px-4 py-2 text-xs font-semibold text-white hover:bg-primary-500 transition-colors"
                                        >
                                            Take related quiz
                                        </Link>
                                    )}
