            ...
            <div className="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
                ...
                {(page.intro as any).how_we_chose_these_breeds && (
                    <section className="mb-10">
                        <h2 className="text-2xl font-bold text-brand-navy mb-3">How we chose these breeds</h2>
                ...
                {page.breed_cards && page.breed_cards.length > 0 && (
                    <section className="mb-12 space-y-6">
                        <h2 className="text-2xl font-bold text-brand-navy mb-4">Top breeds on this list</h2>
                        {page.breed_cards.map((breed, i) => (
                            <div
                                key={`${breed.breed_slug}-${i}`}
                                className="bg-white p-6 rounded-card shadow-card border border-brand-border flex flex-col md:flex-row gap-6 hover:border-brand-teal/50 transition duration-300"
                            >
                                <div className="flex-1">
                                    ...
                                    <p className="text-brand-navy/90 mb-2">{breed.one_line_summary}</p>
                                    <p className="text-brand-navy/60 mb-3 font-medium">
                                        {breed.why_it_works_for_this_lifestyle}
                                    </p>
                                    {breed.major_caveats.length > 0 && (
                                        <ul className="list-disc pl-5 text-brand-navy/60 text-sm space-y-1">
                                    ...
                                    <Link
                                        href={`/breeds/${breed.breed_slug}`}
                                        className="mt-3 inline-block text-brand-teal font-bold hover:text-brand-teal/80 hover:underline"
                                    >
                ...
                {/* Case study */}
                {page.case_study && (
                    <section className="mb-12 bg-white p-6 rounded-card border border-brand-border shadow-card">
                        <h2 className="text-2xl font-bold text-brand-navy mb-3">{page.case_study.title}</h2>
                ...
                {/* CTA */}
                <div className="mt-12 bg-brand-teal rounded-card p-8 text-center text-white shadow-lg border border-brand-teal/20">
